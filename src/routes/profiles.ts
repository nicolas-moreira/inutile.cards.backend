...
import { JWTPayload, IProfileLink, IProfileSocialLink, IProfileTheme } from '../types/index.js';
...
export async function profileRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get('/public/:slug', async (request: FastifyRequest<{ Params: { slug: string } }>, reply: FastifyReply) => {
    try {
  ...
  // Update profile
  fastify.put('/me', { preHandler: [authenticateToken] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const jwtPayload = request.user as JWTPayload;
  ...
  // Update slug
  fastify.put('/me/slug', { preHandler: [authenticateToken] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const jwtPayload = request.user as JWTPayload;
  ...
  // Add link
  fastify.post('/me/links', { preHandler: [authenticateToken] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const jwtPayload = request.user as JWTPayload;
  ...
  // Reorder links
  fastify.put('/me/links/reorder', { preHandler: [authenticateToken] }, async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const jwtPayload = request.user as JWTPayload;
      const body = z.object({
      ...
      const reorderedLinks = body.linkIds.map((id, index) => {
        ...
        return null;
      }).filter(Boolean) as IProfileLink[];
