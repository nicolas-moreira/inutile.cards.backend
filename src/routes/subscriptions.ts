...
import { Subscription, UserSubscription, User } from '../models/index.js';
...
export async function subscriptionsRoutes(fastify: FastifyInstance) {
      ...
      try {
        ...
        if (activeCount > 0) {
          return reply.status(400).send(
            errorResponse(
              `Impossible de supprimer cet abonnement. ${activeCount} utilisateur(s) actif(s) l'utilisent.`
            )
          );
        }
  ...
  fastify.get('/users/all', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const userSubscriptions = await UserSubscription.find()
        .populate('userId')
  ...
  fastify.get('/stats/overview', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      activeUserSubs.forEach((userSub) => {
        const sub = userSub.subscriptionId as any;
