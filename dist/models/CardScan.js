...
const cardScanSchema = new Schema({
    cardId: {
        type: String,
        required: true,
        ref: 'ClientCard',
...
// Static methods
cardScanSchema.statics.findByCardId = function (cardId) {
    return this.find({ cardId }).sort({ scanDate: -1 });
};
cardScanSchema.statics.findByUserId = function (userId) {
    return this.find({ userId }).sort({ scanDate: -1 });
};
cardScanSchema.statics.findBySerialNumber = function (serialNumber) {
    return this.find({ serialNumber: serialNumber.toUpperCase() }).sort({ scanDate: -1 });
};
cardScanSchema.statics.getCardStats = async function (cardId) {
...
cardScanSchema.statics.getUserStats = async function (userId) {
...
export const CardScan = mongoose.model('CardScan', cardScanSchema);
//# sourceMappingURL=CardScan.js.map