...
export async function financeRoutes(fastify) {
    ...
    fastify.get('/overview', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
    ...
    // Get payment cards
    fastify.get('/payment-cards', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
    ...
    // Add payment card
    fastify.post('/payment-cards', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
    ...
    // Remove payment card
    fastify.delete('/payment-cards/:cardId', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
    ...
    fastify.put('/payment-cards/:cardId/default', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
            const { cardId } = request.params;
            const finance = await UserFinance.findOne({ userId: jwtPayload.userId });
    ...
    // Get subscription
    fastify.get('/subscription', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
            const finance = await UserFinance.findOne({ userId: jwtPayload.userId });
    ...
    // Get bills
    fastify.get('/bills', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
    ...
    // Get physical cards
    fastify.get('/physical-cards', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
    ...
    // Order physical card
    fastify.post('/physical-cards', { preHandler: [authenticateToken] }, async (request, reply) => {
        try {
            const jwtPayload = request.user;
    ...
    fastify.put('/physical-cards/:cardId', { preHandler: [authenticateToken, requireAdmin] }, async (request, reply) => {
        try {
            const { cardId } = request.params;
