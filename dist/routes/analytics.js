import { User, Profile, Order, ClientCard, CardScan } from '../models/index.js';
...
export async function analyticsRoutes(fastify) {
    ...
    fastify.get('/', async (request, reply) => {
        try {
            ...
            // Get total counts
            const [totalUsers, totalProfiles, totalCards, totalOrders, totalScans, activeCards,] = await Promise.all([
                User.countDocuments(),
            ...
            const [newUsers, newProfiles, newCards, newOrders, periodScans,] = await Promise.all([
            ...
            const [previousUsers, previousScans, previousOrders,] = await Promise.all([
                User.countDocuments({
    ...
    // Top profiles
    fastify.get('/top-profiles', async (request, reply) => {
        try {
            ...
            topCards.forEach(card => {
                ...
                if (foundCard && foundCard.profileId) {
                    const profileId = typeof foundCard.profileId === 'string'
                        ? foundCard.profileId
                        : foundCard.profileId._id.toString();
    ...
    fastify.get('/devices', async (request, reply) => {
        try {
            ...
            const scans = await CardScan.find({ scannedAt: { $gte: startDate } }, { userAgent: 1 });
            let mobileCount = 0;
            let desktopCount = 0;
            let tabletCount = 0;
            scans.forEach(scan => {
